...
describe 'repose::filter-uri-stripper' do
  before { stub_resources }

  let(:chef_run) { ChefSpec::SoloRunner.new.converge('repose::default', described_recipe) }
  let(:template) { chef_run.template('/etc/repose/uri-stripper.cfg.xml') }
  ...
  it 'creates a template at /etc/repose/uri-stripper.cfg.xml' do
    expect(chef_run).to create_template('/etc/repose/uri-stripper.cfg.xml')
  end
end
