...
describe 'repose::filter-merge-header' do
  before { stub_resources }

  let(:chef_run) { ChefSpec::SoloRunner.new.converge('repose::default', described_recipe) }
  let(:template) { chef_run.template('/etc/repose/merge-header.cfg.xml') }
  ...
  it 'creates a template at /etc/repose/merge-header.cfg.xml' do
