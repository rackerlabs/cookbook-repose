...
describe 'repose::filter-ip-identity' do
  before { stub_resources }

  let(:chef_run) { ChefSpec::SoloRunner.new.converge('repose::default', described_recipe) }
  let(:template) { chef_run.template('/etc/repose/ip-identity.cfg.xml') }
  ...
  it 'creates a template at /etc/repose/ip-identity.cfg.xml' do
    expect(chef_run).to create_template('/etc/repose/ip-identity.cfg.xml')
  end
end
