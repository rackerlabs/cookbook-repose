...
describe 'repose::filter-rate-limiting' do
  before { stub_resources }

  let(:chef_run) { ChefSpec::SoloRunner.new.converge('repose::default', described_recipe) }
  let(:template) { chef_run.template('/etc/repose/rate-limiting.cfg.xml') }
  ...
  it 'creates a template at /etc/repose/rate-limiting.cfg.xml' do
    expect(chef_run).to create_template('/etc/repose/rate-limiting.cfg.xml')
  end
end
