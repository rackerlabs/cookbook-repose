...
describe 'repose::filter-header-translation' do
  before { stub_resources }

  let(:chef_run) { ChefSpec::SoloRunner.new.converge('repose::default', described_recipe) }
  let(:template) { chef_run.template('/etc/repose/header-translation.cfg.xml') }
  ...
  it 'creates a template at /etc/repose/header-translation.cfg.xml' do
    expect(chef_run).to create_template('/etc/repose/header-translation.cfg.xml')
  end
end
